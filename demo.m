...
index_h=20:30; %%specify which host SNPs to be included in the interaction tests. In this example, we only test the 20th - 30th SNPs from the "sequence_host.txt" file. In general, we would recommend users to run marginal tests first, and then select a small subset (say, tens) of host SNPs with significant p-values. Identify the IDs for these SNPS (i.s. the row number in the "input/sequence_host.txt" file) and then specify "index_p" using these IDs. 

index_p=50:60; %%specify which host SNPs to be included in the interaction tests. In this example, we only test the 50th - 60th SNPs from the "sequence_pathogen.txt" file. In general, we would recommend users to run marginal tests first, and then select a small subset (say, tens) of pathogen SNPs with significant p-values. Identify the IDs for these SNPS (i.s. the row number in the "input/sequence_pathogen.txt" file) and then specify "index_h" using these IDs.
